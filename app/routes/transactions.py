...
def admin_get_all_transactions(request: Request):
    ...
    try:
        transactions_db = (
            db.query(TransactionModel).order_by(TransactionModel.id.desc()).all()
        )
        logger.info(
            "Admin %s retrieved %d transactions.", admin_id, len(transactions_db)
        )
...
def admin_create_transaction(request: Request, transaction: TransactionAdd):
    ...
    try:
        participants = (
            db.query(UserModel).filter(UserModel.id.in_(transaction.participants)).all()
        )
        ...
        new_transaction = TransactionModel(
            ...
            items=[ItemModel(name=i.name, price=i.price) for i in transaction.items],
        )
        ...
        db.rollback()
        logger.exception("Failed to create transaction. (Admin id: %s)", admin_id)
        raise
...
def admin_edit_transaction(
    ...
    try:
        ...
        if not transaction:
            logger.warning(
                "Admin %s attempted to edit a nonexistent transaction (%d)",
                admin_id,
                transaction_id,
            )
        ...
        if edited.items is not None:
            transaction.items = [
                ItemModel(name=i.name, price=i.price) for i in edited.items
            ]
...
def admin_delete_transaction(request: Request, transaction_id: int):
    ...
    try:
        ...
        if not transaction:
            logger.warning(
                "Admin %s attempted to delete a nonexistent transaction (%d).",
                admin_id,
                transaction_id,
            )
            raise HTTPException(404, "Transaction not found")
        ...
        db.commit()
        logger.info("Transaction %s was deleted by Admin %d.", transaction_id, admin_id)
...
def get_my_transactions(request: Request):
    ...
    try:
        ...
        logger.exception("Failed to retrieve the transactions of User %s.", user_id)
        raise
...
def create_transaction(request: Request, transaction: TransactionAdd):
    ...
    try:
        ...
        logger.info("User %s created a new Transaction: %d.", user_id, transaction.name)
        return new_transaction

    except Exception:
        db.rollback()
        logger.exception("User %s failed to create a new Transaction.", user_id)
        raise
...
def edit_transaction(request: Request, transaction_id: int, edited: TransactionEdit):
    ...
    try:
        ...
        if not transaction:
            logger.warning(
                "User %s attempted to edit a nonexistent Transaction %d.",
                user_id,
                transaction_id,
            )
            raise HTTPException(404, "Transaction not found")
...
def delete_transaction(request: Request, transaction_id: int):
    ...
    try:
        ...
        if not transaction:
            logger.warning(
                "User %s attempted to delete a nonexistent Transaction %d.",
                user_id,
                transaction_id,
            )
            raise HTTPException(404, "Transaction not found")
        ...
        db.delete(transaction)
        db.commit()
        logger.info("User %s deleted Transaction %d.", user_id, transaction_id)
